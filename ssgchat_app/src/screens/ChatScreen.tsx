...
import { joinChatRoom, enterChatRoom, exitChatRoom } from '../services/chatService';
import { useSpaceExitAlert } from '../hooks/useNotificationHandler';
...
export default function ChatScreen({ userId, nickname }) {
  ...
  useSpaceExitAlert(spaceId);

  useFocusEffect(
    React.useCallback(() => {
      const join = async () => {
        const joined = await getManualJoinList();
        if (joined.includes(spaceId)) {
          await joinChatRoom(spaceId, userId, nickname);
        } else {
